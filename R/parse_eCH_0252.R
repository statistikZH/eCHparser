...
#'   relevant_voteInfo_nodes <- which(grepl(paste0(doi, collapse = "|"), domainofOnfluenceType)) + 2
#'
#'   # Stop if there are no relevant votes (i. e. relevant_voteInfo_nodes)
#'   if (length(relevant_voteInfo_nodes) == 0) {
...
#'
#'
#'   relevant_voteInfo_list <- lapply(seq_along(relevant_voteInfo_nodes), function(relevant_index) {
#'
#'     # Define node of interest
#'     voteInfo_node <- xml2::xml_children(node_voteBaseDelivery)[relevant_index]
#'
#'
#'     ### Meta Data --------------------------------------------------------------
#'
#'
#'     # Define vote node
#'     vote_node <- xml2::xml_find_first(voteInfo_node, ".//vote")
#'
#'     vode_df <- parse_node(vote_node)
#'
#'
#'
#'     ### Results ----------------------------------------------------------------
...
read_voteInfo <- function(xml_node, index, canton_id, polling_day){ # canton_id and polling_day are obsolete i think...
  ...
  if(grep("language", names(vote_info))) {

    language_parent_names <- names(vote_info)[grep("language", names(vote_info))]
