...
#'     if (
#'       "otherIdentification" %in% xml2::xml_name(xml2::xml_parent(children_2_nested)) ||
#'       "namedElement" %in% xml2::xml_name(xml2::xml_parent(children_2_nested)) ||
#'       "subtotalInfo" %in% xml2::xml_name(xml2::xml_parent(children_2_nested)) ||
#'       "language" %in% xml2::xml_name(children_2_nested) ||
#'       "domainOfInfluenceType" %in% xml2::xml_name(children_2_nested)
#'       ) {
#'
#'
#'       # STAND HIER: JETZT MÜSSTEN DIE EINZELNEN FÄLLE DEFINIERT WERDEN (ANALOG LANGUAGE NODES), ANSCHLIESSEND, FALLS DIE EINZELNEN FÄLLE VORHANDEN SIND, MIT EIGENEN FUNKTIONEN GEPARST, ANDERNFALLS NULL OBJECTS ERSTELLT
#'
#'
